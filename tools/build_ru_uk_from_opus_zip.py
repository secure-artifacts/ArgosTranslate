"""
用 Tatoeba OPUS Marian 权重（ct2-marian-converter）构建 ru→uk 包并安装。
比 ct2-transformers-converter 与 Argos 兼容性更好。
"""
...
OPUS_URL = (
    "https://object.pouta.csc.fi/Tatoeba-MT-models/rus-ukr/opus-2021-02-19.zip"
)
PACKAGE_VERSION = "1.1"
ZIP_BASE = "translate-ru_uk-1_1"
...
def main() -> int:
    ...
    cache = _ROOT / "data" / "cache" / "ru-uk-build" / "opus-zip"
    ...
    run = _ROOT / "data" / "cache" / "ru-uk-build" / "run"
    ct2_dir = run / "ct2-marian-ru-uk"
    ...
    (pkg_root / "README.md").write_text(
        "# Russian -> Ukrainian (OPUS-MT Tatoeba)\n", encoding="utf-8"
    )

    if not args.skip_install:
        old = _ROOT / "data/local/argos-translate/packages/translate-ru_uk-1_0"
        if old.is_dir():
            shutil.rmtree(old)
        _install(pkg_root)
        ...
        t = ru.get_translation(uk)
        for s in ("включить", "отключить"):
